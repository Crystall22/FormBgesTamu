    ...
    <div class="card shadow-sm border-0">
        ...
        <div class="card-body">
            <form id="modemForm" action="{{ route('customerservice.modem.update', $modem->id) }}" method="POST">
                ...
                <div class="mb-3 {{ $modem->provider_modem == 'other' ? '' : 'd-none' }}" id="manual_provider_container">
                    ...
                    <input type="text" name="manual_provider" id="manual_provider" class="form-control"
                           value="{{ $modem->provider_modem == 'other' ? $modem->manual_provider : '' }}"
                           {{ $modem->provider_modem == 'other' ? 'required' : '' }}>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (providerSelect.value) {
            ...
            if (providerSelect.value === 'other') {
                ...
                manualProviderContainer.classList.add('d-none');
                manualProviderInput.required = false;
                if (providerSelect.value === 'huawei') {
                    serialInput.maxLength = 16;
                    serialInput.placeholder = 'Masukkan 16 digit';
